    ...
    if ( hash != NULL ) {
        for ( idx = 0; idx < hash->length; idx++ ) {
            for ( e = hash->table[idx]; e != NULL; ) {
                next = e->next;
                if ( relfunc != NULL && e->value.ptr != NULL ) {
                    relfunc(e->value.ptr);
                    e->value.ptr = NULL;
                }
                cel_free( e );
...
static cel_hashmap_node_t **create_node_blocks(uint_t __blocks)
{
    ...
    cel_hashmap_node_t **table = ( cel_hashmap_node_t ** ) 
        cel_calloc( sizeof( cel_hashmap_node_t * ), __blocks );
    ...
    cel_hashmap_t *hash, char *key, cel_hashmap_rcb_fn_t rfunc)
{
    ...
    for ( e = hash->table[idx]; e != NULL; prev = e, e = e->next ) {
        if ( key == e->key || strcmp( key, e->key ) == 0 ) {
            ...
            if ( rfunc != NULL && e->value.ptr != NULL ) {
                rfunc( e->value.ptr );
                e->value.ptr = NULL;
            }
...
CEL_API void cel_hashmap_clear( cel_hashmap_t *hash, cel_hashmap_rcb_fn_t rfunc)
{
    register uint_t idx;
    cel_hashmap_node_t *e, *next;
...
CEL_API int cel_ihashmap_init( cel_ihashmap_t *map, int opacity, float factor )
{
    return cel_hashmap_init(map, opacity, factor);
}

CEL_API int cel_ihashmap_destroy( cel_ihashmap_t *map )
{
    return cel_hashmap_destroy(map, NULL);
}
...
// increase the value associated with the specified key.
CEL_API int cel_ihashmap_inc( cel_ihashmap_t *hash, char *key, int inc)
{
    cel_hashmap_node_t *e;
    uint_t idx = cel_hash(key) % hash->length;
...
// decrease the value associated with the specified key.
CEL_API int cel_ihashmap_dec( cel_ihashmap_t *hash, char *key, int dec)
{
    cel_hashmap_node_t *e;
    uint_t idx = cel_hash(key) % hash->length;
...
CEL_API void cel_ihashmap_clear( cel_ihashmap_t *map )
{
    cel_hashmap_clear(map, NULL);
}
